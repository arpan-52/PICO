...
const double SEC2RAD = PI / (12.0 * 3600.0);
...
// Function to calculate GST at 0h UT
...
double calculateLST(double gstAt0hUT, double utHours, double longitude) {
    ...
    double lst = fmod(gst + longitude, 2 * PI);
    return lst;
}

// Function to calculate UVW coordinates
void calculateUVW(const Vector3& antenna1, const Vector3& antenna2, double ha, double dec, double latitude, Vector3& uvw) {
    ...
    uvw.x = -sinHA * cosLat - cosHA * sinLat * sinDec;
    uvw.y = cosHA * cosLat - sinHA * sinLat * sinDec;
    uvw.z = cosDec * cosLat * sinHA + sinDec * cosLat * cosHA;

    uvw.x = uvw.x * baseline.x + uvw.y * baseline.y + uvw.z * baseline.z;
...
{1760876.87, 5758469.67, 2072457.16}
    ...
    double ra = 13; // Right Ascension in degrees
    double dec = 33.0; // Declination in degrees

    double gstAt0hUT = calculateGST0hUT(2024, 8, 1); // GST at 0h UT for the given date

    // Convert RA from degrees to radians for the calculations
    ra = degreesToRadians(ra * 15.0); // RA in radians

    // Convert startHour from IST to UT
    double startHourUT = startHour - 5.5; // IST is UTC+5:30, so subtract 5.5 hours
    if (startHourUT < 0) {
        startHourUT += 24.0; // Handle negative hour values (wrap around)
    }
    ...
    double observationDuration = 4; // total observation time in seconds
    ...
    for (int step = 0; step <= numSteps; ++step) {
        ...
        double lst = calculateLST(gstAt0hUT, utHours, longitude);

        for (size_t i = 0; i < sizeof(antennas) / sizeof(Vector3); ++i) {
            for (size_t j = i + 1; j < sizeof(antennas) / sizeof(Vector3); ++j) {
                Vector3 uvw;
                double ha = lst * 15.0 - radiansToDegrees(ra); // Hour angle in degrees
                ha = degreesToRadians(ha); // Convert to radians

                calculateUVW(antennas[i], antennas[j], ha, dec, latitude, uvw);
